...
static void init_stats() {
  ...
  if (shm_fd == -1) {
    ...
    shm_fd =
        shm_open(STATS_SHARED_MEMORY, O_CREAT | O_RDWR, SHARED_MEMORY_PERMS);
    newly_created = true;
  ...
  current_stats =
      mmap(NULL, sizeof(stats), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
...
stats *get_stats() {
  ...
  // intento de abrir la memoria compartida. Si falla, es porque no esta, y por
  // ende uso el output NULL de la funcion para imprimir disinto en mi manager.
  int shm_fd = shm_open(STATS_SHARED_MEMORY, O_RDWR, SHARED_MEMORY_PERMS);
  if (shm_fd == -1) {
    return NULL;
  }
...
void cleanup_stats() { shm_unlink(STATS_SHARED_MEMORY); }
